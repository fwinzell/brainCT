...
from monai.losses import DiceLoss, GeneralizedDiceLoss, TverskyLoss, SSIMLoss
...
class GenSegModule(object):
    ...
    def _training_step(self, batch, batch_idx):
        ...
        target = batch["mri"]
        labels = batch["seg"]
        inputs, target, labels = inputs.to(self.device), target.to(self.device), labels.to(self.device)
        outseg, recon = self.model(inputs)

        loss = self.loss_module(outseg, labels, recon, target)

        # Metrics
        if self.sigmoid:
            pred = self.binarize(torch.sigmoid(outseg))
        else:
            pred = self.binarize(outseg)

        dice_score = self._calculate_dice(pred, labels)
        ssim = calculate_ssim(recon, target)

        return loss, pred, dice_score, ssim
    ...
    def train(self):
        for epoch in range(self.max_epochs):
            ...
            for batch_idx, batch in enumerate(tr_loop):
                self.optimizer.zero_grad()
                loss, pred, dice_score, ssim = self._training_step(batch, batch_idx)

                self.writer.add_scalar('train_loss', loss, global_step=self.n_iter)
                total_loss += loss.item()
                for i, c in enumerate(self.labels):
                    self.writer.add_scalar(f"train_{c}_dsc", dice_score[i].item(), global_step=self.n_iter)
                self.writer.add_scalar("train_ssim", ssim.item(), global_step=self.n_iter)
            ...
            val_dsc, val_ssim = self.validate(epoch)
            if self.lr_schedule == "onplateau":
                self.scheduler.step(val_dsc)
            else:
                self.scheduler.step()
    ...
    def validate(self, ep):
        ...
        dscs = torch.zeros(self.n_classes, len(self.val_loader))
        ssim = torch.zeros(len(self.val_loader))
        with torch.no_grad():
            val_loop = tqdm(self.val_loader)
            for batch_idx, batch in enumerate(val_loop):
                dsc, ss = self._validation_step(batch, batch_idx)
                ssim[batch_idx] = ss
        ...
        mean_ssim = torch.mean(ssim)
        self.writer.add_scalar("val_ssim", mean_ssim, global_step=ep)
